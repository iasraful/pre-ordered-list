...
export default function CreatePreorderForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Failed to create preorder:", err);
      setError("Failed to create preorder. Please check your inputs and try again.");
      setIsSaving(false);
    }
  };
    ...
    <div className="w-full max-w-4xl mx-auto py-8 px-4 flex flex-col gap-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-zinc-200/80 rounded-2xl shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="px-8 py-5 border-b border-zinc-100 bg-white">
          <h2 className="text-sm font-bold text-zinc-800">Preorder details</h2>
          <p className="text-zinc-800 text-[11px] mt-1">These values appear in the preorders list.</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[250px_1fr] gap-4 px-8 py-5 border-b border-zinc-100 items-start">
            ...
            <div>
              <input
                ...
                required
                disabled={isSaving}
                className="w-full max-w-md px-3 py-1.5 text-xs border border-zinc-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-zinc-900 focus:border-zinc-900 disabled:opacity-60 transition-all text-zinc-800"
              />
            </div>
          </div>
